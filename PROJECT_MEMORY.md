...
3. **フロー駆動型アーキテクチャ（SSOT: Single Source of Truth）:**
   ...
   - フロントエンド（`index.html`）も共通の `renderFlowStepHTML` によって、デバイスメタデータ（💡, ❄️, 🔥, 🤖, 🔔 等）の自動解決とIF構文（`alt_route` + インラインアイコン）を100%自動描画する。
   - **画一的な構文ルール:**
     - 通常ステップ: **`「デバイス名」を 操作`** `[条件]`（例: `「リビング」を オン`、`「エアコン」を 冷房26℃ [体感24℃以上]`、`「通知」を オン [不在確認]`）。
     - IF条件ステップ: 左アイコン `alt_route` ＋ 文章内インラインアイコン ＋ **`が オン`**。
     - トリガー: **`「デバイス名」が 値`** `[条件]`（例: `「在宅確認」が 変更 [在宅→不在]`、`「時間」が 06:30 [日本の平日（土日祝除く）]`）。
