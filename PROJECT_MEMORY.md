...
- 照明: リモコンAPI (全灯、常夜灯、明るさ上下) / 詳細シート(`light-sheet`)は「アクション」(全灯/常夜灯)ドロップアップメニューに統一
- クリーナー: Eufy RoboVac G30 (開始/一時停止/帰還/探す) / 詳細シート(`cleaner-sheet`)は「アクション」(開始/停止/ホーム)および「吸引力」ドロップアップ、ボタンアイコン色は`text-neutral-400`に統一
- デスクトップPC: IP `192.168.0.20` / MAC `a8:a1:59:60:6f:c0` / WoLブート / SSH電源制御 / 詳細シート(`pc-sheet`)は「電源オプション」(スリープ/再起動)ドロップアップに統一
...
- APK: `Nova Assist v1.3` (versionCode 15, versionName "1.3", 単一統合クライアント)
