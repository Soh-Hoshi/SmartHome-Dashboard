...
3. **シーン・オートメーションの表記ルール:**
   - 画一的な構文: **`「デバイス名」を 操作`**（例: `「リビング」を オン`、`「エアコン」を 冷房26℃`、`「気象」を 取得`）。
   - シーン詳細モーダル: セクション見出し **`フロー`**。
   - オートメーション詳細モーダル: セクション見出し **`トリガー`** と **`フロー`**（十分な間隔 `space-y-6` を確保）。
...
3. **鍵トラッカー（Tile Mate BLE Bluetooth）:**
   - MAC: `18:26:88:50:69:91` / `30:F7:75:1F:0E:20` (Tile, Inc. UUID: `0000feed-0000-1000-8000-00805f9b34fb`)
