...
### 4.3 外出時消し忘れ通知の極限シンプル化
- **消灯時スキップ**: 在宅 ➔ 外出 変化時、照明・エアコン・ヒーター等の電気がすべて消えている場合は通知を一切送信しない（稼働機器がある場合のみ送信）。
- **説明文の全廃**: 「〜が稼働したままです。消灯しますか？」などの冗長な説明文を全廃し、タイトル「お出かけですか？」・本文「リビング照明」等の稼働機器名のみ表示。
- **ボタンの整理**: 「そのまま」ボタンを削除。絵文字やアプリアイコンを排除し、「いってきます」「novaへ指示」のテキストボタン2つのみに最適化。

### 4.4 現在の稼働状態
- **systemd サービス**: `dashboard.service`（Active: running, port 8080）。
- **Git リポジトリ**: すべての変更をコミットし、`origin/main` にプッシュ済み（working tree clean）。
