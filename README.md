...
### 2. ⚡ オートメーション（日本の祝日判定対応）
- **日本の祝日法・振替休日に完全対応**したバックグラウンド・スケジューラー内蔵。
- **「目覚め」**: 日本の平日（土日祝除く）朝 06:30 にリビング照明を自動点灯。
- **詳細モーダル**: 「トリガー（時間・曜日条件）」と「フロー（実行アクション）」を画一的な構文でわかりやすく表示。
