...
10. **クリーナー詳細アクションメニュー化 (2026-09-13)**:
    - 詳細シート(`cleaner-sheet`)の1段目操作ボタン群を、PC起動OSやヒーターモードと同様のドロップアップメニュー(`cleaner-action-dropup-menu`)およびフル幅トリガーボタン(`btn-trigger-cleaner-action`)へ刷新。
    - ドロップアップ項目は「開始」「停止」「ホーム」の3種。
    - アクションは即時実行コマンド（非トグル・非状態値）であるため、トリガーボタンのラベルは「アクション / 操作を選択」の固定表示とし、メニュー内のチェックマークも非表示にしてシンプル化。
    - 操作実行時はアシスタントトースト通知で状況をフィードバック。
